...
A revisão sistemática identificou 17 estudos de alta relevância (relevance\_score $\geq 4.0$) 
que constituem a base empírica para as fases subsequentes deste projeto de TCC.

\section{Fluxo PRISMA 2020}

A Figura~\ref{fig:prisma-flow} ilustra o fluxo de seleção PRISMA. O processo iniciou com a identificação de 9.431 registros. Após a remoção de duplicatas e triagem inicial, 1.883 estudos foram avaliados quanto à elegibilidade. Destes, 1.866 foram excluídos com base no critério de relevância ($\text{score} < 4.0$), resultando na inclusão final de 17 estudos (taxa de inclusão de $\sim$0,18\%).
...
Total incluído (relevance $\geq$ 4.0) & 17 \\
...
\begin{table}[htbp]
\centering
\caption{Síntese dos 17 estudos incluídos na revisão sistemática.}
\label{tab:sintese-17-estudos}
\scriptsize
...
Termos menos frequentes, mas relevantes: \textit{AI/Artificial Intelligence} (2; 11,8\%), \textit{Intelligent Tutoring} (1; 5,9\%), além de abordagens específicas presentes em títulos (e.g., \textit{Reinforcement Learning}, \textit{Knowledge Graph}, \textit{Computer Animation}) que não aparecem como palavras-chave padronizadas.

\textbf{Nota metodológica}: As porcentagens referem-se a presença do termo em pelo menos um estudo; termos não listados nos campos \texttt{keywords} mas citados em títulos ou textos (como \textit{Personalized Learning}) não foram incluídos nesta contagem para manter consistência reprodutível.
...
Outras/Específicas (ACO, CRF, Knowledge Graph, Sentiment, Animation, Jigsaw, XAI) & 6 & 35,3\% \\
\hline
\end{tabular}
\end{table}

\textbf{Nota metodológica}: Contagem considera presença binária da categoria por estudo; categorias não são mutuamente exclusivas e percentuais podem somar $>100\%$. A categoria ``Outras/Específicas'' agrega técnicas singulares de baixa frequência. Tabela reconciliada com `referencias.bib` (remoção de estudo não incluído e correção de anos 2023 e 2020).
...
\end{tabular}
\end{table}
\textbf{Nota metodológica}: Cada estudo classificado em uma única finalidade para preservação de proporções; estudos com descrições híbridas receberam atribuição pela finalidade predominante explícita.
