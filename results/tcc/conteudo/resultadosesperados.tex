...
Antes da tabela detalhada, três padrões orientam sua leitura. Primeiro, houve predominância de estudos voltados à predição de desempenho ou à classificação de proficiência. Segundo, diferentes algoritmos foram empregados, com recorrência de \textbf{\textit{Random Forest}}, SVM e redes neurais. Terceiro, os resultados não são diretamente comparáveis em todos os casos, pois os estudos utilizaram populações, instrumentos, variáveis e desenhos metodológicos distintos. Dos 16 registros incluídos, 15 são estudos empíricos e um protocolo/proposta sobre ensino inclusivo de matemática \cite{Imperatrice2025_6921} não apresenta resultado empírico concluído e foi preservado somente como registro contextual. A avaliação MMAT foi aplicada de forma preliminar em nível documental aos 15 registros empíricos; o protocolo contextual não é aplicável ao MMAT empírico. Nove textos primários foram revisados, enquanto cinco registros permanecem apoiados em resumo ou metadados. As conclusões de qualidade devem considerar essas limitações das fontes.

Assim, os 15 registros empíricos constituem um escopo provisório, pois parte das avaliações permanece apoiada em resumo ou metadados. O estudo de Zeng \cite{Zeng2025_6923} tem identidade bibliográfica e ano corroborados, mas continua limitado à informação de resumo enquanto o texto integral não for recuperado.
...
Käser Jacober (2014) \cite{KaserJacober2014} & Aprendizagem matemática assistida por computador & Tutoria; avaliação; predição & Personalização & O estudo descreve sistema adaptativo baseado em modelo de habilidades matemáticas e estudos com usuários \\
...
O MMAT 2018 foi utilizado como instrumento de apreciação metodológica conforme o desenho de cada estudo \cite{MMAT2018}. As respostas Y (``Sim''), N (``Não'') e CT (``Não é possível determinar'') são mantidas por critério, sem média, ranking ou categoria geral. A avaliação documental foi aplicada aos 15 registros empíricos: nove textos primários foram revisados e cinco registros receberam julgamentos provisórios baseados em resumo ou metadados. O protocolo/proposta sobre ensino inclusivo de matemática \cite{Imperatrice2025_6921} foi mantido fora da avaliação MMAT empírica. O instrumento não é reduzido a uma nota, e decisões sem apoio no texto integral devem ser interpretadas como provisórias.
...
Para tornar o procedimento transparente, a Tabela~\ref{tab:mmat-avaliacao-atual} apresenta a avaliação documental preliminar dos 16 registros incluídos, distinguindo os 15 registros empíricos do protocolo contextual. Ela explicita a base da avaliação, conserva ``CT'' quando a fonte disponível não permite uma decisão e identifica o protocolo como não aplicável ao MMAT empírico. A tabela não atribui nota geral nem ordena os estudos.
