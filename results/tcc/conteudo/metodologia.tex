...
Execução automatizada das consultas nas quatro APIs, resultando em \textbf{11.904 registros} no banco consolidado. Cada registro inclui metadados bibliográficos (título, autores, ano, \textit{venue}, DOI/URL) e, quando disponível, resumo (\textit{abstract}). A base \path{research/systematic_review.sqlite} foi adotada como fonte de verdade para os estados e contagens desta consolidação. O snapshot foi persistido sem registros marcados pela flag \texttt{is\_duplicate}; por isso, não houve remoção adicional registrada no fluxo descrito neste capítulo. A auditoria de DOI é apresentada separadamente para não confundir a flag operacional com unicidade bibliográfica.
...
O \textit{pipeline} foi projetado para ser reexecutável mediante comandos CLI (\textit{Command Line Interface}), com parâmetros e artefatos versionados. A reprodução exata das respostas das APIs não é garantida, pois fontes externas, expiração do cache e metadados podem mudar. Para esta consolidação, as contagens e os estados devem ser reproduzidos a partir da base atual e dos exports derivados, sem presumir que uma nova coleta produza o mesmo snapshot:
