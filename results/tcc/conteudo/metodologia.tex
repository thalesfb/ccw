...
Este trabalho empregou uma Revisão Sistemática da Literatura para identificar e analisar técnicas computacionais aplicadas ao ensino de matemática. O \textit{Cochrane Handbook for Systematic Reviews of Interventions} \cite{CochraneHandbook} e referências do \textit{Institute of Medicine} (IOM) foram consultados como referências gerais de transparência e sistematicidade. Essa consulta não implica adoção integral de um protocolo Cochrane ou de um modelo formal do IOM.
...
\begin{itemize}
    \item Inglês: \textit{adaptive}, \textit{personalized}, \textit{tutoring}, \textit{analytics}, \textit{mining}, \textit{machine learning}, \textit{ai}, \textit{assessment}, \textit{student modeling}, \textit{predictive}, \textit{intelligent tutor} e \textit{artificial intelligence};
    \item Português: \textit{adaptivo}, \textit{personalizado}, \textit{tutor}, \textit{analitica}, \textit{mineração}, \textit{aprendizado de máquina}, \textit{ia}, \textit{avaliação}, \textit{modelagem do aluno}, \textit{preditivo}, \textit{tutor inteligente} e \textit{inteligência artificial}.
\end{itemize}
...
O processo de seleção foi documentado segundo as etapas de identificação, triagem, elegibilidade operacional e retenção apresentadas pelo fluxo PRISMA 2020. Neste trabalho, a etapa denominada elegibilidade no fluxo corresponde a uma priorização operacional baseada em metadados, resumo e critérios documentais; ela não deve ser confundida com uma confirmação universal de elegibilidade em texto completo.
...
Após a deduplicação determinística, 11.877 registros foram submetidos à triagem. Desses, 9.391 foram excluídos na triagem e 2.486 avançaram para a etapa de priorização operacional reportada no fluxo como elegibilidade.

\subsection{Priorização operacional e elegibilidade documental}

A etapa de priorização operacional utilizou uma pontuação de \textbf{relevância temática}, distinta da avaliação de qualidade metodológica. A heurística considerou:
