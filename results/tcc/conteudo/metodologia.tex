...
Após a avaliação de pertinência, foram incluídos \textbf{16 registros}, representando uma taxa de retenção de aproximadamente \textbf{0,13\%} em relação ao total identificado (16/11.904). Desses registros, \textbf{15 são classificados como empíricos} para a síntese, enquanto o ID 6921 é um protocolo/proposta retido para contexto, fora da síntese empírica. A interpretação do ID 6918 é limitada pela divergência entre o ano informado na fonte institucional e o ano registrado nos metadados, conforme discutido no capítulo de resultados.
...
Foi definido um procedimento de avaliação crítica de qualidade metodológica utilizando o \textit{Mixed Methods Appraisal Tool} (MMAT) versão 2018 \cite{MMAT2018}. O MMAT foi selecionado por sua capacidade de avaliar diferentes desenhos de estudo (qualitativos, quantitativos e mistos) de forma padronizada, adequando-se à heterogeneidade metodológica identificada nesta revisão. A avaliação foi aplicada aos 15 registros empíricos; o protocolo contextual 6921 permanece fora da avaliação empírica. Cada estudo é descrito por decisões e justificativas para cada critério. Quando a fonte disponível não permite uma decisão segura, utiliza-se ``Não é possível determinar'', e essa limitação é considerada na interpretação dos resultados.
...
\begin{enumerate}
    ...
    \item \textbf{Classificação do desenho de estudo}: Os 15 registros empíricos aplicáveis foram categorizados conforme seu desenho metodológico (qualitativo, quantitativo randomizado, quantitativo não-randomizado, quantitativo descritivo ou métodos mistos). A classificação inicial automatizada é apenas um ponto de partida e deve ser confirmada no texto primário; o protocolo 6921 permanece separado.
