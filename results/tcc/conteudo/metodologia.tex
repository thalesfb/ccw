...
Na aplicação do procedimento de seleção descrito neste trabalho, o seletor automatizado exigiu apenas os
...
para todos os registros incluídos. Por isso, a seleção não deve ser usada para
afirmar que esses requisitos foram verificados individualmente. O protocolo/proposta sobre ensino inclusivo de matemática \cite{Imperatrice2025_6921} foi
mantido entre os registros retidos, mas, por ser protocolo/proposta sem resultado
empírico concluído, foi separado como registro contextual e não integra a síntese
...
As consultas automatizadas nas quatro APIs resultaram em \textbf{11.904 registros brutos}. Cada registro inclui metadados bibliográficos (título, autores, ano, \textit{venue}, DOI/URL) e, quando disponível, resumo (\textit{abstract}). Antes da triagem, foram removidos \textbf{27 registros} por identidade determinística (25 excedentes de DOI normalizado e 2 de URL exata; um grupo de URL é misto quanto à presença de DOI). Assim, \textbf{11.877 registros} seguiram para a triagem. Títulos repetidos foram mantidos como candidatos à análise de identidade, pois a semelhança textual isolada não prova duplicidade bibliográfica.

\subsection{Triagem}

Na triagem, os registros foram avaliados por regras operacionais aplicadas aos metadados e aos resumos disponíveis. Os mecanismos de identificação de possíveis duplicatas são descritos na Seção de Deduplicação e não devem ser confundidos com as exclusões de triagem. Os motivos registrados para as exclusões foram:
\begin{itemize}
...
Essas categorias totalizam 9.413 ocorrências de motivos. Como 27 registros
redundantes foram removidos antes da contagem do fluxo PRISMA, a contagem
analítica de exclusões na triagem é 9.391. As categorias devem ser interpretadas
como razões operacionais de seleção, e não como avaliação de qualidade
metodológica dos trabalhos.

No fluxo PRISMA após a deduplicação determinística, \textbf{11.877 registros} foram avaliados na triagem. \textbf{9.391 registros} foram excluídos nessa etapa, e \textbf{2.486 registros} avançaram para a análise de elegibilidade. Essas contagens representam registros únicos no fluxo analítico; as 9.413 ocorrências de motivos incluem os 27 registros redundantes retirados antes da contagem PRISMA.
...
A implementação soma os componentes ponderados, limita o resultado ao intervalo de 0 a 10 e produziu valores observados entre 0 e 4,5. O limiar operacional foi definido como pontuação de relevância maior ou igual a 4,0. DOI, citações e acesso aberto não funcionam como medida de qualidade científica nessa implementação: apenas a presença de DOI e um indicador simplificado de veículo entram no componente de metadados. Por isso, o limiar não garante elegibilidade metodológica, qualidade dos estudos ou validade dos resultados, e os candidatos foram submetidos à avaliação de pertinência.

Nesta etapa foram excluídos \textbf{2.470 registros} (99,4\% dos 2.486 registros avaliados) e 16 foram incluídos. A pontuação foi um filtro operacional de aderência temática; essa contagem não constitui avaliação de qualidade metodológica.

\subsection{Inclusão}

Após a avaliação de pertinência, foram incluídos \textbf{16 registros}, representando uma taxa de retenção de aproximadamente \textbf{0,13\%} em relação ao total identificado (16/11.904). Desses registros, \textbf{15 são classificados como empíricos} para a síntese, enquanto um protocolo/proposta sobre ensino inclusivo de matemática \cite{Imperatrice2025_6921} foi retido para contexto, fora da síntese empírica.

\section{Deduplicação}

A deduplicação foi realizada antes da contabilização do fluxo PRISMA. A identidade bibliográfica foi verificada por DOI normalizado e por URL exata normalizada. A similaridade de título foi usada apenas como diagnóstico complementar: TF-IDF (\textit{Term Frequency--Inverse Document Frequency}) com n-gramas de caracteres é comparado pela similaridade do cosseno, que varia de 0 a 1, com limiar operacional de $0{,}9$. A semelhança de título, isoladamente, não foi suficiente para remover um registro.

\begin{enumerate}
    \item \textbf{Por DOI}: o DOI é normalizado (minúsculas, formas comuns de resolvedor, remoção do prefixo ``doi:'' e pontuação editorial terminal), e ocorrências com a mesma identidade são representadas uma única vez;
    \item \textbf{Por URL}: a URL exata normalizada é usada como chave independente para identificar repetições;
    \item \textbf{Por título}: a similaridade TF-IDF pelo cosseno $\geq 0{,}9$ indica um candidato à análise, mas não determina exclusão automática.
\end{enumerate}

Foram identificados 25 grupos de DOI repetido (50 linhas, 25 excedentes) e 2 grupos de URL repetida (4 linhas, 2 excedentes), totalizando 27 registros redundantes retirados antes da triagem. A análise textual também encontrou 177 grupos de título normalizado (434 linhas, 257 excedentes); após a remoção por DOI/URL, permanecem 154 grupos (386 linhas, 232 excedentes) apenas por título. Esses candidatos não foram removidos automaticamente porque não provam identidade bibliográfica.
...
O código, os parâmetros de busca, os exports tabulares, o resumo PRISMA, as
bibliografias, os registros de auditoria e o manifesto de reprodutibilidade
foram versionados no repositório. Esses artefatos permitem reconstituir as
contagens e a composição reportadas sem distribuir o banco local. A variação
das APIs e dos metadados externos é uma limitação da reprodutibilidade; por
isso, as contagens reportadas devem ser interpretadas a partir dos artefatos
versionados.

Os parâmetros de configuração, os instrumentos de avaliação e os artefatos
derivados estão documentados no repositório. Credenciais e o banco operacional
local permanecem fora do versionamento. A bibliografia completa do TCC é
composta por \path{results/tcc/referencias.bib} e
\path{results/tcc/referencias_pedagogicas.bib}. As referências de fundamentação
teórica e metodológica não são contadas como estudos derivados da revisão nem
incluídas na bibliografia dos estudos selecionados.

\section{Avaliação da Qualidade Metodológica}

Foi definido um procedimento de avaliação crítica de qualidade metodológica utilizando o \textit{Mixed Methods Appraisal Tool} (MMAT) versão 2018 \cite{MMAT2018}. O MMAT foi selecionado por sua capacidade de avaliar diferentes desenhos de estudo (qualitativos, quantitativos e mistos) de forma padronizada, adequando-se à heterogeneidade metodológica identificada nesta revisão. A avaliação foi aplicada aos 15 registros empíricos; o protocolo contextual sobre ensino inclusivo de matemática \cite{Imperatrice2025_6921} permanece fora da avaliação empírica. Cada estudo é descrito por decisões e justificativas para cada critério. Quando a fonte disponível não permite uma decisão segura, utiliza-se ``Não é possível determinar'', e essa limitação é considerada na interpretação dos resultados.
...
A avaliação foi conduzida por um único revisor, com base em nove textos primários e, nos demais casos, em resumos e metadados disponíveis. As decisões sem apoio no texto integral são apresentadas como provisórias. Estudos com limitações metodológicas não foram excluídos automaticamente, mas tiveram essas limitações consideradas na síntese narrativa e na discussão dos achados.

A avaliação por critério é apresentada na Seção~\ref{sec:qualidade-estudos}, com a indicação da base documental utilizada e das limitações de interpretação.
