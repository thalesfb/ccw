...
O repositório também contém uma implementação técnica do pipeline do protótipo, acompanhada por testes sintéticos automatizados. Essa implementação demonstra propriedades de processamento e de proteção contra falhas previstas no código, mas não equivale a uma execução com dados educacionais reais, a uma validação de eficácia preditiva ou a uma avaliação de aprendizagem. O capítulo, portanto, descreve a especificação que orienta uma futura execução e mantém essa implementação técnica separada de qualquer alegação de resultado educacional.
...
Os requisitos funcionais descrevem as capacidades que uma futura implementação deverá oferecer para apoiar a interpretação docente de evidências educacionais. Inicialmente, o protótipo deverá importar dados documentados e relacionados a objetivos curriculares, preservando os metadados necessários à leitura de sua procedência. A partir desses dados, deverá organizar evidências de desempenho por habilidade ou descritor, de modo que a análise mantenha relação explícita com o contexto curricular em que os registros foram produzidos.

Na etapa de análise, o protótipo deverá produzir estimativas de proficiência acompanhadas de incertezas e limitações e apresentar resultados individuais e agregados ao professor. Essas saídas deverão ser acompanhadas de uma explicação das variáveis relacionadas a cada estimativa, sem serem apresentadas como diagnósticos completos de aprendizagem. O fluxo também deverá permitir revisão humana das recomendações produzidas, para que o professor possa confrontar as evidências computacionais com a situação da turma e decidir sobre sua pertinência pedagógica. Por fim, deverá exportar dados, parâmetros e resultados para auditoria, assegurando que as decisões de processamento e as saídas apresentadas possam ser examinadas posteriormente.
