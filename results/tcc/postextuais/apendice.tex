...
Este apêndice apresenta as 27 recomendações do PRISMA 2020 para relato de revisões sistemáticas e indica onde cada informação é abordada no TCC. A indicação de uma localização não equivale, por si só, à comprovação de completude metodológica; as limitações e os itens não aplicáveis são explicitados no corpo do trabalho.
...
11b & Desvios do protocolo & Relatar desvios do protocolo e justificativas & Cap. 3 (ausência de registro prospectivo declarada) \\
\hline
...
17 & Viés de publicação & Avaliação de viés de publicação & Cap. 4, Seção Limitações (limitação reconhecida; sem teste formal) \\
\hline

18 & Certeza da evidência & Avaliação de certeza/qualidade da evidência & Cap. 4, Seção MMAT (certeza formal não estimada) \\
\hline
...
\textbf{Status do relato: itens localizados no manuscrito; a localização não implica conformidade integral.}

Os itens metodológicos que exigem maior cautela são tratados da seguinte forma:

\begin{enumerate}
    \item \textbf{Avaliação metodológica (itens 9 e 14):} Apreciação documental preliminar por critérios com o \textbf{Mixed Methods Appraisal Tool (MMAT)} versão 2018 \cite{MMAT2018}, apresentada no Cap. 4 sem pontuação agregada. O protocolo ou proposta contextual retido não é avaliado como evidência empírica.
    \item \textbf{Certeza da evidência (item 18):} Não foi estimada uma classificação formal de certeza da evidência; essa ausência e a condição preliminar do MMAT são declaradas no Cap. 4.
    \item \textbf{Viés de publicação (item 17):} A limitação decorrente da cobertura das fontes e da ausência de um teste formal é discutida no Cap. 4.
    \item \textbf{Limitações da evidência e da revisão (itens 20 e 21):} O Cap. 4 descreve as limitações da evidência e do processo, incluindo revisor único, disponibilidade desigual de textos completos e heterogeneidade dos estudos.
\end{enumerate}
