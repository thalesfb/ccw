...
14 & Risco de viés nos estudos & Apresentar avaliações de risco de viés para cada estudo & Cap. 4, Seção Qualidade Metodológica (reaplicação preliminar registrada; adjudicação pendente) \\
\hline
...
18 & Certeza da evidência & Avaliação de certeza/qualidade da evidência & Cap. 4, Seção Qualidade Metodológica (MMAT preliminar registrado; fontes e adjudicação pendentes) \\
\hline
...
\section*{Nota sobre o baseline e o conjunto incluído}

No snapshot operacional vigente, o banco operacional contém 11.904 registros. A deduplicação determinística por DOI/URL retirou 27 registros redundantes; no fluxo PRISMA, 11.877 foram avaliados na triagem, 9.391 foram excluídos e 2.486 avançaram à elegibilidade. Nessa etapa, 2.470 foram excluídos e 16 registros foram retidos operacionalmente: 15 classificados provisoriamente como empíricos, com o 6918 em hold temporal, e o protocolo/proposta contextual 6921. O PTC foi avaliado apenas como proposta não publicada e registrava 17 incluídos; esse número é uma referência documental, não uma execução científica anterior. O snapshot atual registra 23 candidatos e 7 overrides manuais, dos quais 4 ainda aguardam adjudicação de escopo. A comparação entre as versões é exclusivamente documental e deve ser lida junto dos artefatos versionados, não como simples troca numérica.

Os 15 registros empíricos são uma classificação provisória para fins de rastreabilidade. O ID 6918 está em hold porque a fonte oficial da ETH informa 2014, em conflito com o ano 2025 do snapshot; o ID 6921 permanece contextual e fora do MMAT empírico. A avaliação final depende da resolução dessas pendências, da recuperação das fontes e da adjudicação.
...
\textbf{Status de conformidade: aderência parcial; não se declara 27/27 enquanto a avaliação metodológica do conjunto atualizado não for consolidada.}

O checklist registra o estado atual do relato. O método para a avaliação de risco de viés foi definido no item 9, e os itens 17, 20 e 21 foram tratados qualitativamente. A avaliação estudo a estudo do conjunto atual foi iniciada e registrada em ledger próprio: nove textos primários foram revisados externamente, cinco estudos permanecem em nível de abstract/metadados, um está em hold por conflito de ano e recuperação de fonte (ID 6918), e um protocolo/proposta sem resultado empírico concluído (ID 6921) foi marcado como não aplicável ao MMAT empírico. Por isso, os itens 14 e 18 permanecem provisórios até a conclusão das fontes e da adjudicação. Os itens 9, 14, 17, 18, 20 e 21, que na fase PTC estavam pendentes, foram tratados na versão TCC da seguinte forma:

\begin{enumerate}
    \item \textbf{Avaliação de qualidade metodológica (itens 9, 14, 18):} Definição do procedimento com o \textbf{Mixed Methods Appraisal Tool (MMAT)} versão 2018 \cite{MMAT2018} para os 15 registros empíricos retidos. O protocolo/proposta 6921 permanece separado como registro contextual e não aplicável ao MMAT empírico. A reavaliação preliminar está registrada em \path{research/data/mmat\_reassessment\_current.csv}; a tabela final permanece pendente de fontes primárias, localizadores por critério e adjudicação.
